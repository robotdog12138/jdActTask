...
function queryJdActivityById() {
    request({
            method: "POST",
            timeout: 10000,
            url: "https://wq.jd.com/bases/manfan/QueryActivityById?activityId=" +
                count +
...
function txtWTFromJson(res) {
    ...
    if (typeof res == "object") {
        ...
        if (data && data.success) {
            ...
            let newArr = appstoreSKU.filter(item => {
                return prodSkuList.includes(item.id)
            })
            console.log(
                "活动内容:" +
                displayTitle +
